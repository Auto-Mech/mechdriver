     ...
     +        ispecies.eq.61) then
      ...
      if(iaspace.eq.1)then
         ...
         if(ibarr.eq.2)then
            ...
            call activespace(nbonds,nlps,nstates,neltot)
...
c         close(99)
         ...
         goto 999
      else if (word3.eq.'RESCALE4'.and.ispecies.eq.0)then
         irescale=4
         open (unit=99,file='./me_files/reac1_en.me',status='old')
         read(99,*)hl_react_en
         close(99)
...
c            stop
     ...
     $                 status='unknown')
                  open (unit=22,file='./data/ts_asl1_step2.inp',
     $                 status='unknown')
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine activespace(nbonds,nlps,nstates,neltot)
...
cc    first check if number of electrons is even or not
      ...
      if(icheck.ne.0)then
         ...
         write(7,*)'with odd number of electrons ',neltot
      ...
      do j=1,numclosedtot
         ...
         write(101,990)nclose,nopen,neltot,nstates
      enddo
      close(101)
