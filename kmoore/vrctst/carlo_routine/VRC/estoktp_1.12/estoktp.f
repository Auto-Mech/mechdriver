...
132           continue
              read (21,'(A70)') comline1
              if(iend1.eq.1) write (23,*) comline1
              if (comline1.EQ.'End1'.or.comline1.eq.' End1')iend1=1
              if (comline1.EQ.'End2'.or.comline1.eq.' End2')go to 133
              goto 132
133           continue
              close(21)
              close(22)
...
cc now correct for ZPE differences at two levels of theory 
     ...
     +         status='unknown')
         ...
         if(ts100zpe_cas.ne.0)prodzpecas=ts100zpe_cas
...
cc noew read energy of products
...
cc now read optimized coordinates
      ...
      if(ibarr.eq.2)then
         close(23)
         open (unit=23,file='./output/prod1_opt.out',status='old')
      endif
...
c            write (*,*) intcoor(natom1p+itau-ntau1),taumn,taumx
      ...
      if(ibarr.eq.2)then
         close(24)
         open (unit=24,file='./output/prod2_opt.out',status='unknown')
      endif
...
c      endif
      ...
      do j=1,numclosedtot
         ...
         write(101,990)nclose,nopen,neltot,nstatew
      enddo
      close(101)
...
cc
 990  format('{multi;closed,',I2,';occ,',I2,';wf,',I3,',1,0;state,',I1'
     $ ;maxiter,40}')
...
c      do j=1,natomt
c         write(*,*)'atomlabel is ',atomlabel(j)
c      enddo
c      do j=1,ncoord
c         write(*,*)'coord is ',intcoor(j),xint(j)
c      enddo
...
c      nfreq=3*natom-nhind-1-6
      ...
      write(107,*)'    Frequencies[1/cm] ',nfreqw
      write(108,*)'    Frequencies[1/cm] ',nfreqw
      write(109,*)'    Frequencies[1/cm] ',nfreqw
      write(107,8010) (freqintproj(j),j=1,nfreqw)            
      write(108,8010) (freqintproj(j),j=1,nfreqw)
      write(109,8010) (freqproj(j),j=1,nfreqw)
...
cc write third vrc-tst file, structure.inp (of bimol prods/reacs)
      ...
      if(ibarr.eq.2)then
         ...
         open(unit=12,file='./geoms/prod1_l1.xyz',status='unknown')
         ...
         open(unit=12,file='./geoms/prod2_l1.xyz',status='unknown')
...
cc write molpro VRC template using default template
 ...
 105     continue
      ...
      if(word2.eq.'ENCORR')then
         open(unit=15,file='../100/me_files/potcorr_hl.me',
     $ status='unknown')
         read(15,*)vinfen
         close(15)
c         vinfen=100.000
      ...
      if (ispin.eq.1) write (11,*) comline1
      goto 100
 201  continue
...
cc read other coordinates if natoms gt 2
      ...
      do j=1,natom1
         read (15,*) cjunk,coox,cooy,cooz
         write(7,121)j,cjunk,coox,cooy,cooz
         if(j.eq.isite)then
            xsite1=coox
            ysite1=cooy
            zsite1=cooz
...
c         theta1=(acos(xsitep1/sqrt(xsitep1**2+ysitep1**2)))*180./3.14159
      ...
      if(abs(zsitep1).lt.0.01)then
         ...
         phi1=(atan2(sqrt(xsitep1**2+ysitep1**2),zsitep1))*180./3.14159
      endif

c      write(7,*)'test',sqrt(xsitep1**2+ysitep1**2)  
      write(7,*)'Positioning of pivot atoms of fragment 1'
      write(7,*)'pivot1 coor after trasl are: ,',xsitep1,ysitep1,zsitep1
      write(7,*)'pivot1 polar coords are: ,',rho1,theta1,phi1

cc if natom2>1 we computed the pivot atoms also for fragment 2
...
c      stop
      ...
      write(11,*)'#  4--->2'
      write(11,*)
      write(11,102)xsite1,ysite1,zsite1
      write(11,106)xsite1,ysite1,zsite1
      write(11,*)
      if(natom2.eq.1)then
         write(11,*)'PivotPoints     1'
         write(11,*)'Frame 0 0 0 0'
         write(11,*)
         write(11,*)'x3 = 0 y3 = 0 z3 = 0'
         write(11,*)
         write(11,*)'# interpointal distances'
         write(11,*)'#'
         write(11,*)
         ...
         write(11,*)'Cycles 6'
         write(11,*)
         write(11,*)'r = ( 8.0, 7.5, 7., 6.5, 6.0, 5.75, 5.5, 5.0) '
         write(11,*)'d1 = (0.1, 0.3, 0.5)'
         write(11,107)phi1
         write(11,108)theta1
         write(11,109)phi1
         write(11,110)theta1
         ...
         write(11,*)'Frame 0 0 0 0'
         write(11,*)
         write(11,112)xsite4,ysite4,zsite4
         write(11,116)xsite4,ysite4,zsite4
         ...
         write(11,*)'Distances'
         write(11,*)
         write(11,*)'r11 = r-d1-d2'
         write(11,*)'r21 = r-d1-d2'
         write(11,*)'r12 = r-d1-d2'
         write(11,*)'r22 = r-d1-d2'
         write(11,*)
         write(11,*)'Conditions 0'
         write(11,*)
         write(11,*)'Cycles 11'
...
cc reference infinte energy is assigned to npoints+1 position
      ...
      do j=1,npoints
         potco(j)=potgr(j)-potgr(npoints+1)-potg(j)+potg(npoints+1)
         potco(j)=potco(j)+pothl(j)-pothl(npoints+1)-pothl(j)
     $            +pothl(npoints+1)
         potco(j)=potco(j)*627.5
...
c      enddo
 ...
 102  format('x1  = ',F5.2,' +d1*sin(p1)*cos(a1) y1 = ',F5.2,
     + '+d1*sin(p1)*sin(a1) z1 = ',F5.2,1X,' +d1*cos(p1)')
 103  format('../',A8,'/me_files/potcorr_geom.me')
 104  format('../',A8,'/me_files/potcorr_hl.me')
 106  format('x2  = ',F5.2,' -d1*sin(p2)*cos(a2) y2 = ',F5.2,
     + '-d1*sin(p2)*sin(a2) z2 = ',F5.2,1X,' -d1*cos(p2)')
 107  format('p1 = (',F6.2,')')
 108  format('a1 = (',F6.2,')')
 109  format('p2 = (',F6.2,')')
 110  format('a2 = (',F6.2,')')

 112  format('x3  = ',F5.2,' +d2*sin(p3)*cos(a3) y3 = ',F5.2,
     + '+d2*sin(p3)*sin(a3) z3 = ',F5.2,1X,' +d2*cos(p3)')
 116  format('x4  = ',F5.2,' -d2*sin(p4)*cos(a4) y4 = ',F5.2,
     + '-d2*sin(p4)*sin(a4) z2 = ',F5.2,1X,' -d2*cos(p4)')
 117  format('p3 = (',F6.2,')')
 118  format('a3 = (',F6.2,')')
 119  format('p4 = (',F6.2,')')
 120  format('a4 = (',F6.2,')')
...
cc first determine correction potential for geometry
      ...
      command1="cp -f output/ts_asl1_step2.inp ./level0.dat"
      call commrun(command1)

      command1="grep -v frequencies ./level0.dat > ./level0_molpro.dat"
      call commrun(command1)
...
c      write(99,*)gmem
c      close(99)
c      command1="sed -ie 's/MW/ /' temp.tmp"
c      call commrun(command1)
c      open(unit=99,file='temp.tmp',status='unknown')
c      read(99,*)igmem
c      close(99)
c      write(11,*)'memory,',igmem,',m'
...
c      open(unit=15,file='./output/ts_opt.out',status='unknown')
...
cc assume calculations will be done only for molpro
     ...
     $        ilev,ispecies,iaspace)
      ...
      open(unit=13,file='./me_files/ts_en.me',status='unknown')
      read(13,*)vtotref
      close(13)
      write(12,*)vtotr
      write(12,*)vtotref
