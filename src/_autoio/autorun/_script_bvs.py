...
MOLPRO_2021 = (
    "#!/usr/bin/env bash\n"
    "ulimit -c 0\n"
    "/software/cinetica/molpro_2022.2/bin/molpro "
    "-n {} run.inp -o run.out -d $GAUSS_SCRDIR "
...
MOLPRO_2021_MPPX = (
    ...
    "ulimit -c 0\n"
    "/software/cinetica/molpro_2022.2/bin/molpro "
    "--mppx -n {} run.inp -o run.out -d $GAUSS_SCRDIR"
    "--nouse-logfile --no-xml-output >> "
    "stdout.log &> stderr.log"
)
MOLPRO = (
    "#!/usr/bin/env bash\n"
    "ulimit -c 0\n"
    "molpro -n {} run.inp -o run.out -d $GAUSS_SCRDIR"
    "--nouse-logfile --no-xml-output >> "
    "stdout.log &> stderr.log"
)
MOLPRO_MPPX = (
    "#!/usr/bin/env bash\n"
    "ulimit -c 0\n"
    "molpro --mppx -n {} run.inp -o run.out -d $GAUSS_SCRDIR"
    "--nouse-logfile --no-xml-output >> "
    "stdout.log &> stderr.log"
)
MOLPRO_MREF = (
