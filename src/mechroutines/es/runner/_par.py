...
def qchem_params(method_dct, job=None, geo=None, spc_info=None):
    ...
    ret = INI_PARAM_BUILD_DCT[prog](
        method_dct, prog, job=job, geo=geo, spc_info=spc_info
    )
...
def _gaussian(method_dct, prog, job=None, geo=None, spc_info=None):
        ...
        kwargs.update(
            {
                ...
                "job_options": ["Tight"],
                "feedback": True,
                "errors": [elstruct.Error.OPT_NOCONV],
                "options_mat": [
                    [
                        {"job_options": ["Tight"]},
                        {"job_options": ["Tight"]},
    ...
    if job == elstruct.Job.OPTIMIZATION:
        ...
        irc_pts = method_dct.get("nirc", 30)
        irc_step = 15
...
def _molpro(method_dct, prog, job=None, geo=None, spc_info=None):
    ...
    if job == elstruct.Job.OPTIMIZATION:
        ...
        uhf_str = f"{{uhf,maxit=150;wf,{elec_count},1,{two_spin};orbprint,3}}"
        cas_str = (
            "{casscf,maxit=40;"
...
def _psi4(method_dct, prog, job=None, geo=None, spc_info=None):
    ...
    if job == elstruct.Job.OPTIMIZATION:
        kwargs.update(
            {
                "feedback": False,
                "errors": [elstruct.Error.SYMM_NOFIND],
                "options_mat": [
                    [
