...
def run(tsks, rxns, lvls, spcs):
    """ driver to generate mess file, run mess and convert output to chemkin format
    """

   # Add loop over all of the rxn lvls specified (ultimately move to top level driver)
    ...
    for rxn in rxns:
        ...
        #Print message
        print('||| Reaction {} will compute {} for '.format(rxn, ' and '.join(rxns[rxn]['job'])))
        if rxns[rxn]['products']:
            print('the reaction: {} --> {}'.format(
                ' + '.join([automol.inchi.smiles(spcs[x]['inchi'])
                            for x in rxns[rxn]['reactants']]),
                ' + '.join([automol.inchi.smiles(spcs[x]['inchi'])
                            for x in rxns[rxn]['products']])))
        else:
            print('the species: '.format(rxns[rxn]['reactants']))
        ...
        for tsk in tsks[rxn]:
            ...
            if tsk == 'tsfind':
                ...
                geo, zma = scripts.es.find_ts(
                    run_prefix, save_prefix, rxns[rxn]['reactants'],
                    rxns[rxn]['products'], spcs, running_thy_info, overwrite)
                if not isinstance(geo, str):
                    spcs['ts_' + rxn] = create_spec(geo)
                    spcs['ts_' + rxn]['zmatrix'] = zma
                    print(' Success, ts_{} added to queue'.format(tsk))
                    species_queue.append('ts_' + rxn)
                continue

            for spc in species_queue:
                print('\n  | Task {} \t\t\t Species {}: {}'.format(
                    tsk, spc, automol.inchi.smiles(spcs[spc]['inchi'])))
                ...
                if not 'ts' in spc:
                    scripts.es.geo_ref(
                        spcs[spc], initial_thy_info, running_thy_info,
                        run_prefix, save_prefix, overwrite)
                    scripts.es.remove_imag(
                        spcs[spc], lvls[tsks[rxn][tsk][0]], run_prefix,
                        save_prefix, overwrite)
                    scripts.es.run_task(
                        tsk, spcs[spc], lvls[tsks[rxn][tsk][0]],
                        initial_thy_info, run_prefix, save_prefix, overwrite)
                else:
                    scripts.es.ts_geo_ref(
                        spc, spcs, rxns[rxn]['reactants'],
                        rxns[rxn]['products'], initial_thy_info,
                        running_thy_info, run_prefix, save_prefix, overwrite)
                    scripts.es.ts_run_task(
                        tsk, spc, spcs, rxns[rxn]['reactants'],
                        rxns[rxn]['products'], lvls[tsks[rxn][tsk][0]],
                        initial_thy_info, run_prefix, save_prefix, overwrite)
...
if __name__ == "__main__":
    ...
    MSG = """
           ...
           ===  Carlo Cavolotti, Yuri Georgievski, Murat Keceli,        ===
           ==                   Stephen Klippenstein                     ==
           ================================================================
           ==                          ES to k(T,P)                      ==
           ===         Carlo Cavalotti, Sarah Elliott, Andreas Copan,   ===
           ==       Kevin Moore, Yuri Georgievski, Stephen Klippenstein  ==
           ================================================================\n"""
    print(MSG)
