...
import sys
import autofile
from mechlib.filesys.build import zma_fs_from_prefix
...
def _set_orbital_restriction_label(spc_info, thy_info):
    ...
    if thy_info[3] == 'RR':
        ...
        if mul == 1:
            ...
            orb_restr = 'U'
    assert orb_restr is not None, (
        'orbital restriction is not defined for {}'.format(thy_info[3]))
    return orb_restr
...
def combine_spc_info(spc_info1, spc_info2, mval='max'):
    """ Created a spc info for two species where charge and mult
        have been combined
    """

    ich = (spc_info1[0], spc_info2[1])
    chg = spc_info1[1] + spc_info2[1]
    if mval == 'max':
        mult = max([spc_info1[2], spc_info2[2]])
