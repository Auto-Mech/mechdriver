...
def lj_collision_frequency(sig, eps, mass1, mass2,
    ...
    for temp in temps:
        ...
        ioprinter.debug_message('eps', eps)
    ...
    ioprinter.info_message(
        '- Collisional frequencies from LJ parameters', indent=1)
    for temp, val in zlj_dct.items():
        ioprinter.info_message(
            'T = {0} K, zlj = {1:<.3e} cm3/s'.format(temp, val), indent=2)
...
def _calc_edown_expt(alpha_dct):
    ...
    ioprinter.info_message('- Alpha parameters from estimation', indent=1)
    for temp, val in alpha_dct.items():
        ioprinter.info_message(
            'T = {0} K, alpha = {1:<.3f} cm-1'.format(temp, val), indent=2)
...
def calc_n_eff(geo,
    ...
    ioprinter.debug_message('- Rotor Counts for N_eff:', indent=1)
    ioprinter.debug_message('N_pp:{}, N_ps:{}, N_pt:{}, N_pq:{}'.format(
        n_pp, n_ps, n_pt, n_pq), indent=2)
    ioprinter.debug_message(
        'N_ss:{}, N_st:{}, N_sq:{}'.format(n_ss, n_st, n_sq), indent=2)
    ioprinter.debug_message(
        'N_tt:{}, N_tq:{}'.format(n_tt, n_tq), indent=2)
    ioprinter.debug_message('N_qq:{}'.format(n_qq), indent=2)
    ioprinter.debug_message(
        'N_co:{}, N_oo:{}'.format(n_co, n_oo), indent=2)
    ioprinter.debug_message(
        'N_ss_ring:{}, N_rings:{}'.format(n_ss_ring, n_rings), indent=2)
