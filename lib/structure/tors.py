...
def hr_prep(zma, geo, run_tors_names=(), scan_increment=30.0, ndim_tors='1dhr',
    ...
    """
    # Add check if user tors are within the ones that we generate...
...
def get_names_from_dct(spc_dct_i, ndim_tors):
    """ Build the tors name list from 
    """

    if 'tors_names' in spc_dct_i:
        run_tors_names = spc_dct_i['tors_names']
        print('Reading tors names defined by user...')
    elif 'auto_tors_names' in spc_dct_i:
        run_tors_names = spc_dct_i['auto_tors_names']
        if ndim_tors == '1dhr':
